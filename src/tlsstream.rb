...
class TLSStream
  ...
  def initialize(eaptlspackets)
    ...
    eaptlspackets.length.times do |cur_pkt|
      ...
      i = 0
      until seen_change_cipher_spec || i >= cur_records.length
        cur_rec = cur_records[i]
        seen_change_cipher_spec = true if cur_rec.is_a? TLSChangeCipherSpecRecord
        cur_rec.set_handshake_type if cur_rec.is_a? TLSHandshakeRecord
        ...
        if cur_rec.is_a? TLSAlertRecord
          ...
          if cur_pkt + 1 == eaptlspackets.length && cur_rec.alert_code == TLSTypes::Alerts::CLOSE_NOTIFY
            ...
            logger.warn "Seen a TLS Alert #{cur_rec.inspect_alert}"
            @alerted = true
            return
          end
        end
...
class TLSRecord
  ...
  def initialize(version, length, data)
    ...
    if length != data.length
      raise TLSParseError, "TLS Indicated Length (#{length}) did not match actual length (#{data.length})"
    end
...
class TLSHandshakeRecord < TLSRecord
  ...
  def set_handshake_type
    raise TLSParseError, 'The Handshake is too short' if @data.length < 4

    @handshake_type = data[0]
    @handshake_length = data[1] * 256 * 256 + data[2] * 256 + data[3]
    raise TLSParseError, 'The Indicated length did not match the actual length' if @data.length - 4 != @handshake_length
  end
  ...
  def self.parse_handshakes(version, length, data)
    ...
    raise TLSParseError, 'The indicated lengths did not match with the data' unless cur_ptr == data.length
...
class TLSAlertRecord < TLSRecord
  attr_reader :alert_level, :alert_code

  def initialize(version, length, data)
    super
    raise TLSParseError, "The Alert must be exactly 2 Bytes long. Actual length #{data.length}" if data.length != 2
  ...
  def code_string
    TLSTypes::Alerts.get_alert_name_by_code(@alert_code)
  end
end
