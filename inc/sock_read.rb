...
def watchdog(stat_item, thr)
  loop do
    ...
    if last_values[-2, 2].reduce(0, :+).zero?
  ...
  loop do
    ...
    loop do
      ...
      while bytes.length > 11
        ...
        from_length = bytes[i, 2].unpack1('n')
        ...
        from_sock = bytes[i, 4].unpack1('N')
        i += 4

        # Now we fetch the "To" length
        to_length = bytes[i, 2].unpack1('n')
        ...
        to_sock = bytes[i, 4].unpack1('N')
        i += 4

        break if bytes.length < i + 4

        radius_length = bytes[i + 2, 2].unpack1('n')
        ...
        BlackBoard.pktbuf.synchronize do
          ...
          BlackBoard.pktbuf << { source: path, request: request, from: from, from_sock: from_sock,
                                 to: to, to_sock: to_sock, pkt: radius_pkt }
